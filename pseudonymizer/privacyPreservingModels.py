...
from typing import *
import pandas as pd
# ./pseudonymizer/pseudonymizer/privacyPreservingModels.py
...
class PrivacyPreservingModel:
    ...
    def K_Anonymity(self, K: int, sensitive_attributes):
        ...
        for group_key, index_value in self.equivalent_class.items():
            if len(self._dataframe.loc[index_value, sensitive_attributes]) >= K:
                print( self._dataframe.loc[index_value, sensitive_attributes], "\n")
            else:
                continue
